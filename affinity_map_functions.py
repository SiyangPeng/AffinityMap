...
def sigmoid_curve_gen(x_val, y_values, z_values, global_zmin, b_seed):   
    ...
    try:
        ...
        params_full.add('b', value=b_seed, min=-10, max=3)
...
def remove_correspond_0(list1, list2):
    ...
    indices_to_remove = [index for index, value in enumerate(list1) if value == 0]
...
def remove_vehicle_test(fit_result,data_by_x,protein_data,global_zmin):
    ...
    for x_value, values in data_by_x.items():
        ...
        if p_value < 0.001:
            # Take out 0 values
            y_values, z_values = remove_correspond_0(np.log10(values['y']), values['z'])
